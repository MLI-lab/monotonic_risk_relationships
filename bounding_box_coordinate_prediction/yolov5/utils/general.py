...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    t = time.time()
    # (added lines)
    indices = [torch.arange(start=0, end=prediction.shape[1], device=prediction.device)] * prediction.shape[0]

    output = [torch.zeros((0, 6), device=prediction.device)] * prediction.shape[0]
    for xi, x in enumerate(prediction):  # image index, image inference
        # (added lines)
        # print("non_max_suppression(): #1 x.shape {}, indices[xi].shape {}".format(x.shape, indices[xi].shape))

        # Apply constraints
        x[((x[..., 2:4] < min_wh) | (x[..., 2:4] > max_wh)).any(1), 4] = 0  # width-height
        x = x[xc[xi]]  # confidence
        # (added lines)
        indices[xi] = indices[xi][xc[xi]]
        # print("non_max_suppression(): #3 x.shape {}, indices[xi].shape {}".format(x.shape, indices[xi].shape))
        ...
        if multi_label:
            ...
            # (added lines)
            indices[xi] = indices[xi][i]

            x = torch.cat((box[i], x[i, j + 5, None], j[:, None].float()), 1)
        else:  # best class only
            conf, j = x[:, 5:].max(1, keepdim=True)
            # (added lines)
            indices[xi] = indices[xi][conf.view(-1) > conf_thres]
        ...
        if classes is not None:
            # (added lines)
            indices[xi] = indices[xi][x[:, 5:6] == torch.tensor(classes, device=x.device)]
